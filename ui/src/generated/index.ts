...
export type SendCliCommandMutation = {
  sendCliCommand?: Maybe<Pick<CliResponse, 'time' | 'message' | 'command' | 'isError'>>
}
...
export type ConnectionsQuery = {
  connections: Array<Pick<Connection, 'id' | 'name' | 'host' | 'port' | 'protocol' | 'isActive'>>
}
...
export type NamespacedKeysQuery = {
  namespacedKeys: {
    allKeys: Array<Pick<Key, 'key'>>
    namespaced: Array<Pick<NameSpacedKeys, 'name'> & { keys: Array<Pick<Key, 'key'>> }>
  }
}
